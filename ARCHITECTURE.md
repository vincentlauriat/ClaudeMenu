...
| `MenuBarMeter` | Le compteur affiché dans la barre de menus : `week` ou `session`, stocké sous `menubar.meter` | Un `@Published` avec `didSet`, jamais `@AppStorage` : celui-ci est un `DynamicProperty` rafraîchi seulement pendant l'évaluation d'une vue, donc dans un `ObservableObject` il lit une fois et ne republie jamais, et le titre cesserait silencieusement de suivre le réglage |
